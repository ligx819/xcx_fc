...
Page({
      ...
      if (planDetail.lists.length > 0) {
        if (allBasketBalls.length == 0 || (allBasketBalls.length > 0 && allBasketBalls != planDetail.lists)) {
          planDetail.lists.forEach(function (e) {
            var pid = home.checkBetType(e['play_id']);
            var cid = home.checkPickWay(e['bet_mode']);
            var lt = home.listData;
            var nm = lt[pid][cid].name;
            var ball = {
              red: e['red_ball'],
              bet: e['bet'],
              // type: e['play_id'],
  ...
  randomNum: function (e) {
    ...
    var st = wx.getStorageSync('nowSelData') || home.listData.dx.danshi;
    var db = st.balls;
    var bjz = parseInt(that.data.bjz);
    var content = '';
    ...
    for (let k = 0; k < rand; k++) {
      var jx_red = [], name= st.name, jizhu = 1;
      var rb = home.randData(st);
      if (st.id == 'baoduan') {
        var start = rb[0].join('');
        var end = rb[1].join('');
        var dd = {};
        parseInt(end) < parseInt(start) ? (dd.start = end, dd.end = start) : (dd.start = start, dd.end = end);
        jx_red.push(dd.start);
        jx_red.push(dd.end);
        jx_red = jx_red.join(' | ');
        jizhu = home.countBetNum(dd,st);
      } else {
        for (let i = 0; i < rb.length; i++) {
          ...
          for (var j = 0; j < rb[i].length; j++) {
            ...
            bb.push(db[i][val].ball);
          }
          jx_red.push(bb.sort().join(','));
        }
        jx_red = jx_red.join(' | ');
        jizhu = home.countBetNum('null', st);
      }
      var one = {
        red: jx_red,
        cid: isNaN(st.id) ? home.checkPickWay(st.id) : st.id,
        pid: isNaN(st.pid) ? home.checkBetType(st.pid) : st.pid,
        name: name,
        bet: jizhu
      };
            ...
            if (data.return_code == 'SUCCESS' && data.return_msg == 'SUCCESS') {
              if (data.return) {
                if (data.return.code == 'error') {
                  ...
                  app.log(data.return, that.route);
                  var url = '';
